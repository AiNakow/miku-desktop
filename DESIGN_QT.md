...
| `def`           | 18   | 8   | ✅    | 默认待机         |
| `hello`         | 10   | 8   |       | 桌宠启动时播放   |
| `bye`           | 14   | 8   |       | 托盘菜单「退出」时播放 |
| `excited`       | 8    | 10  |       | 随机             |
| `shy`           | 16   | 8   |       | 左键点击随机池   |
| `angry`         | 5    | 8   |       | 1 秒内左键点击 4 次（播放期间不可被其他动画打断） |
| `cry`           | 12   | 8   |       | `fall` 后静止 3 秒；或 `angry` 触发超过 5 次 |
| `heart`         | 10   | 8   |       | 随机             |
| `question`      | 7    | 8   |       | 左键点击随机池   |
| `dizzy`         | 19   | 8   |       | 持续拖拽并晃动超过 3 秒 |
| `jump`          | 8    | 10  |       | 随机             |
| `rolling`       | 15   | 8   |       | 托盘「跳舞」     |
| `rotate`        | 10   | 8   |       | 随机             |
| `hairflip`      | 5    | 8   |       | 随机             |
| `sleep`         | 14   | 6   |       | 托盘「睡觉」循环；21:00 后至次日 07:00 前 10 分钟无鼠标点击自动进入循环 |
| `watch`         | 8    | 8   |       | 随机             |
| `drink`         | 33   | 10  |       | 随机             |
| `eatcake`       | 15   | 8   |       | 托盘「喂食」     |
| `listenmusic`   | 4    | 6   |       | 随机             |
...
> 说明：运行时对所有动画统一应用约 `0.82x` 的播放倍率，因此实际观感速度会略慢于表中 FPS。
...
```
 ...
 ┌──────────────┐   mouseRelease ──► fall ──►
 │   Dragging   │   (moved)
 │  (lift 循环) │   mouseRelease ──► 左键随机动画池（含 shy/question，不含 hello） ──► idle
 └──────────────┘   (click, <300ms, no move)
 ...
 托盘触发:
   「喂食」  → eatcake → idle
   「跳舞」  → rolling → idle
     「睡觉」  → sleep（循环播放，直到鼠标点击）
     「退出」  → bye     → 进程退出

 特殊触发:
     1 秒内左键点击 4 次 → angry
     fall 结束后保持 idle 3 秒 → cry
     angry 累计触发超过 5 次 → cry
        本地时间 21:00 后至次日 07:00 前，连续 10 分钟无鼠标点击 → sleep（循环播放，直到鼠标点击）
     持续拖拽并明显晃动超过 3 秒 → dizzy（在释放时替代 fall）
     angry 播放期间不可被其他动画打断
```
